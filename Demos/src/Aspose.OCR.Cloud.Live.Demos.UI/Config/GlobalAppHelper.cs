...
namespace Aspose.OCR.Cloud.Live.Demos.UI.Config
{
    ...
    public class GlobalAppHelper
    {
		...
		public GlobalAppHelper(HttpContext hc, HttpApplicationState appState, string SessionID, string language)
		{
			...
			AsposeOCRCloudContext.atcc = new AsposeOCRCloudContext(hc); // sync the context
		...
		private void initResources(string ResourcesFile, string SessionID)
		{
			...
			if (AsposeOCRCloudContext.Current.Cache[SessionID] == null)
			{
				...
				AsposeOCRCloudContext.Current.Cache.Add(
				   SessionID,
					resources,
